...
<table class="el-table pushWrap" cellspacing="0" cellpadding="0" >
  ...
  <tr  class="el-table-column">
    ...
    <td style="padding-right: 20px;" colspan="2">
        ...
        시
        <el-select v-model="minutes" size="small" style="width: 100px;">
            <el-option v-for="i in 10" :value="i-1" :key="i">0{{i-1}}</el-option>
  ...
  <th rowspan="3" class="title_style" >발송 대상</th>
    ...
    <div v-for="index in gender" :key="index.key" style="display: inline-block; padding: 10px;" >
        <label>
      <input type="checkbox"  :value="index.key" :id="index.key" v-model="targetGender"d>
      {{index.name}}
    </label>
    </div>
    </td>
  ...
  <th rowspan="2" class="title_style" >Push</th>
    ...
    <td style="width: 403px;  padding: 10px 0 10px; border-top: 1px solid #ded5d5">
        <el-input v-model="pushTitle" size="mini" style="width: 500px;"></el-input>
    </td>
  </tr>
  <tr  class="el-table-column">
    <th>Push 본문</th>
    <td>
        <el-input v-model="pushMain" size="mini" style="width: 500px;"></el-input>
    </td>
  </tr>
   <tr class="el-table-column">
  <th class="title_style">페이지</th>
    <th style="width: 150px;border-top: 1px solid #ded5d5">페이지 링크</th>
    <td style="border-top: 1px solid #ded5d5">
       <el-button type="info" @click="dialogTableVisible = true" size="mini">찾아보기</el-button>
...
<el-dialog title="이벤트 페이지 링크 팝업창" :visible.sync="dialogTableVisible">
        <el-form class="searchBoxStyle">
            ...
            <table  class="el-table" style="margin-bottom: 30px;">
            ...
            <tbody class="el-table-column listTable">
                <tr v-for="(entry, key) in results" :value="results.id" :key="results.id">
  ...
  <div class="btnWrap">
    <el-button type="info" size="small" class="submitBtn" @click="addPush()">등록</el-button>
...
<script>
import { GetTargetSexCd, GetTagetAgesCd, GetBoardStates, GetTargetShoesCd, GetActiveAdEvents, AddPushs, GetSendPurposeCd } from '@/api/marketing'
  ...
  export default {
    data() {
      return {
        ...
        sendPurpose: '',
        sendSelect: '',
        sendDate: new Date(new Date().getFullYear(), new Date().getMonth(), (new Date().getDate() + 1 )),
        value2: '',
        value1: '',
        hours: '18',
        minutes: '00',
