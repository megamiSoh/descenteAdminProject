...
<div class="warmWrap" id="vList">
  <h1>상세 정보
    <span v-if="isAuth()">
        <el-button type="success" size="small" @click="editIsActive(sports.isActive)" v-if="sports.isActive" plain>활성화</el-button>
        <el-button type="info" plain size="small" @click="editIsActive(sports.isActive)" v-else>비활성화</el-button>
        </span>
        <span v-else>
        {{sports.isActive ? '활성화' : '비활성화'}}
    </span>
  </h1>
  <el-form>
...
<table class="el-table" cellspacing="0" cellpadding="0" >
  ...
  <th rowspan="3" class="title_style" >기본 정보</th>
    ...
    <td style="width: 403px;  padding: 10px 0 10px;">
      <el-select v-model="sportsSelect" disabled size="small">
  ...
  <tr  class="el-table-column">
    ...
    <th>운동 효과</th>
    <td>
      <el-input :max-length="20" v-model="effectInput" disabled class="effectStyle" size="small"></el-input>
    </td>
  </tr>
  <tr  class="el-table-column">
    <th>운동 강도</th>
  ...
  <tr>
    ...
    <td colspan="3" class="separate" style="padding: 10px 0 10px 0;">
       <label class="file-label">
        <span class="el-button" type="primary" plane size="small" style="display: none;">이미지 추가</span>
                ...
                <table>
                    <tr>
                        <td width="150" style="padding: 20px;">
                            <el-radio v-model="radio" :label="true" disabled>동영상 업로드</el-radio>
                            
                        </td>
                        <td>
                            <el-button  @click="onlyShow = true; mediaOpen()" size="small" class="media_btn" :disabled="true">추가</el-button>
                                        <el-dialog
                                        title="동영상 미리보기"
                                        :visible.sync="videoShow">
                                        <video :src="videoPath" width="100%" controls>
                                        </video>
                                        </el-dialog>
                                        <el-button class="linkA" @click="videoShow = true" size="small" v-if="videoName.length > 0">{{ videoName }}</el-button>
                        </td>
                    </tr>
                    <tr>
                        <td width="150"  style="padding: 20px;">
                            <el-radio v-model="radio" :label="false" disabled>외부 링크</el-radio>
                        </td>
                        <td>
                            <a v-bind:href="linkUrl" style="padding: 10px; display: block;color: blue; text-decoration: underline;" target="_blank">{{linkUrl}}</a>
                            <el-input :disabled="true" v-model="linkUrl"></el-input>
                        </td>
                    </tr>
                </table>
            </el-form>
                    ...
                    <table class="media_result"   cellpadding="0" cellspacing="0">
                        <tbody>
                            <tr>
                                ...
                                <td> 
                                    <el-button size="small" @click="showIntro= true" class="linkA">
                                        {{ data1[0].originName }}
                                    </el-button>
                                    <el-dialog
                                    title="영상 미리보기"
                                    ...
                                    >
                                        <video :src="data1[0].logicalPath"  width="100%" controls></video>
                                    </el-dialog>
                                </td>
            ...
            <tr>
                <td>
                    ...
                    <tr>
                        ...
                        <th class="titleBottom">
                            <el-button @click="mainShow = true" size="small" disabled disabled>변경</el-button>
                        </th>
                    </tr>
                </table>
                </td>
  ...
  <div class="btnWrap">
  <router-link v-bind:to="{path: 'sportsremove'}" v-if="isAuth()">
    <el-button type="info" size="small" class="submitBtn">수정</el-button>
</router-link>
    <router-link v-bind:to="{path: 'sportscontent'}">
...
import  { Addsports, GetActiveWarmups, Exercises, Cooldowns, GetExerKind, PurposeCategory,IntensityCd, PhotoModels, GetPurposeCategor, GetEffectUnitCd, EncodeVideos, GetVideoMenus, GetWarmupPart, getDetail, sportsDetail, editIsActive} from '@/api/sports'
