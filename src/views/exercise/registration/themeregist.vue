...
<table class="el-table" cellspacing="0" cellpadding="0" >
  ...
  <tr  class="el-table-column">
    ...
    <td style="padding: 10px 0 10px;">
        <el-form :inline="true" size="small">
            <el-form-item style="width: 150px;">
             <el-select v-model="muscleAreaCateId"  @change="childDefault()" size="small">
            ...
            <el-form-item>
            <el-select v-model="exerPurpose"  :disabled="muscleAreaCateId === null ? true: false" placeholder="선택해 주세요." size="small">
    ...
    <td rowspan="2">
        <el-input v-model="keyword" class="effectStyle"></el-input> <span class="exam" size="small">예) #다이어트 #10대</span>
    </td>
  </tr>
  <tr  class="el-table-column">
    <th>운동 설명
                ...
                <div class="mediaPopOver">
                    ...
                    <el-form class="searchBoxStyle">
                        ...
                        <el-form :inline="true" size="mini">
                                ...
                                <div>
                                    <el-button  @click="onlyShow = true; mediaOpen()" size="small" >추가</el-button>
                                <span class="themeResults" v-for="lists in data4" :value="lists.id" :key="lists.id">
                                       {{ lists.originName }}
                                   </span>
                                </div>
                            </el-form>
                                        ...
                                        <table class="media_result"   cellpadding="0" cellspacing="0">
                                            <tbody>
                                                <tr>
                                                    ...
                                                    <td v-for="lists in data1"> {{ lists.originName }}</td>
...
<el-dialog
    ...
    <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="coolAdd()" size="small">선택</el-button>
    </span>
    </el-dialog>
<el-dialog
...
>
    ...
    <span slot="footer" class="dialog-footer">
        ...
        <el-button @click="warmAdd()" size="small" type="primary">선택</el-button>
    </span>
</el-dialog>
 ...
 <el-dialog
    ...
    <span slot="footer" class="dialog-footer">
        ...
        <el-button type="primary" @click="mainAdd()" size="small">선택</el-button>
    </span>
</el-dialog>
</div>
  <div class="btnWrap">
    <el-button type="info" size="small" class="submitBtn" @click="themeRegist()">등록</el-button>
    <router-link v-bind:to="{path: 'themecontents'}">
...
<script>
import { addTheme,warmActive, getMainActive, getCoolActive, getExerKind, getIntensity, getModel, getPurposeCategory, getEffectCd, getMedia, GetWarmupPart, GetVideoMenus,} from '@/api/exercise'
...
import { getToken, setToken, removeToken, reToken } from '@/utils/auth'
  export default {
      ...
      return {
        ...
        renderFunc(h, option) {
         ...
         renderFuncMain(h, option1) {
          return <table  class="transStyle" stripe>
          <colgroup>
            <col width="70" />
            <col width="250" />
            <col width="120" />
    ...
    getMedia() {
     ...
     warmActive() {
            ...
            .then(response => {
                ...
                // alert(JSON.stringify(this.warmAct))
                // for(var x in this.warmd){
                //     for ( var i = 0; i <= this.warmAct.length; i++) {
                // this.warmA = this.warmAct[i].filter(item => {
                //     return item.id === warmd[x]
                // })
                // alert(this.warmAct[i].filter(item => {
                //     return item.id === warmd[x]
                // })
                // )
                //     }
    ...
    //    selectVideo(val){
    //       this.videoPath1 =this.results.filter(item => {
    ...
    childDefault1(){
        ...
        //     // var x = JSON.parse(this.restWarm)
        //     alert(typeof(this.restWarm))
        // },
       themeRegist() {
            if(this.exerInput <= 0) {
                this.$notify.error({
            ...
            this.coolArr.push({
                    'restTime' : JSON.stringify(0),  'actions' : coolActions
                })
...
.el-table th div.el-dialog{height: 750px;; overflow-y: scroll; margin-top: 0!important;}
.el-table .cell, .el-table th div {display: block; overflow-y: auto;}
